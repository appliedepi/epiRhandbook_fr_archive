...
Pour rassembler les trois composants (conteneur, séquence et opérations), essayons de tracer une épicurve pour chaque hôpital (voir la page sur les [Epidemic curves]).  
...
outbreak <- incidence2::incidence(   
     ...
     na_as_group = TRUE) # le sexe manquant est son propre groupe

# tracer la courbe d'épidémie
plot(outbreak, # nom de l'objet d'incidence
     fill = "gender", # couleur des barres par sexe
     color = "black", # couleur de contour des barres
     title = "Outbreak of all cases" # titre
     )
```
...
#### Exporter en tant que feuilles Excel {.non numéroté}  
...
* Le chemin du fichier fourni à `export()` est dynamique grâce à l'utilisation de `str_glue()` (voir la page [Characters and strings]) :  
...
Pour extraire les éléments d'une liste par nom ou par position, vous pouvez utiliser des crochets "[[ ]]" comme décrit dans la page [R basics]. Ci-dessous, nous utilisons des doubles crochets pour indexer la liste `t.tests_results` et afficher le premier élément qui est le résultat du test t sur `age`.  
