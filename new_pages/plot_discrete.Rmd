...
```{r}
# Convertir en facteur et définir l'ordre des niveaux pour que "Autre" et "Manquant" soient les derniers.
linelist <- linelist %>% 
  mutate(
...
Voir la page [ggplot basics] pour les principes fondamentaux, et la page [ggplot tips] pour les techniques plus avancées.  
...
Nous créons ensuite le ggplot avec quelques mises en forme supplémentaires :
  ...
  * **Axis flip** : Nous avons inversé les axes avec `coord_flip()` pour pouvoir lire les noms des hôpitaux.
  * **Columns side-by-side** : Ajout d'un argument `position = "dodge"` pour que les barres pour la mort et la récupération soient présentées côte à côte plutôt qu'empilées. Notez que les barres empilées sont la valeur par défaut.
