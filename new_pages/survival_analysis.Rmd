...
*L'analyse de survie* s'attache à décrire pour un individu ou un groupe d'individus donnés, un point d'événement défini appelé **_l'échec_** (apparition d'une maladie, guérison d'une maladie, décès, rechute après réponse à un traitement...) qui survient après une période de temps appelée **_le temps d'échec_** (ou **_le temps de suivi_** dans les études de cohorte/population) pendant laquelle les individus sont observés. Pour déterminer le temps d'échec, il est alors nécessaire de définir un temps d'origine (qui peut être la date d'inclusion, la date du diagnostic...). 

La cible d'inférence de l'analyse de survie est alors le temps entre une origine et un événement. Dans la recherche médicale actuelle, l'analyse de survie est largement utilisée dans les études cliniques pour évaluer l'effet d'un traitement par exemple, ou en épidémiologie du cancer pour évaluer une grande variété de mesures de survie au cancer. 
...
<span style="color : orange ;">**_CAUTION:_** Puisque dans une étude de cohorte réelle, l'information sur le moment de l'origine et la fin du suivi est connue étant donné que les individus sont observés, nous éliminerons les observations où la date d'apparition ou la date de l'issue est inconnue. De même, les cas où la date d'apparition est postérieure à la date de l'issue finale seront supprimés car ils sont considérés comme erronés.</span>

<span style="color : darkgreen ;">**_ASTUCE:_** Étant donné que le filtrage sur une date supérieure à (>) ou inférieure à (<) peut supprimer les lignes avec des valeurs manquantes, l'application du filtre sur les mauvaises dates supprimera également les lignes avec des dates manquantes.</span>

Nous utilisons ensuite `case_when()` pour créer une colonne `age_cat_small` dans laquelle il n'y a que 3 catégories d'âge.

```{r }
#Créer un nouveau jeu de données appelé linelist_surv à partir du jeu de données linelist_case_data.
...
linelist_surv <- linelist_case_data %>% 
  ...
  dplyr::mutate(
       # créer une variable qui s'appelle événement et qui vaut 1 si le patient est décédé et 0 s'il a été censuré à droite
       event = ifelse(is.na(outcome) | outcome == "Recover", 0, 1), 
    
       # créer une variable sur le temps de suivi en jours
       ...
       # l'étape précédente a créé la variable age_cat_small en tant que caractère.
...
<span style="color : darkgreen ;">**_ASTUCE:_** Nous pouvons vérifier les nouvelles colonnes que nous avons créées en faisant un résumé sur le `futime` et un tableau croisé entre `event` et `outcome` à partir duquel la variable `futime` a été créée. Outre cette vérification, c'est une bonne habitude de communiquer la durée médiane de suivi lors de l'interprétation des résultats de l'analyse de survie.</span>

```{r }

summary(linelist_surv$futime)

# croiser la nouvelle variable événement et la variable résultat à partir de laquelle elle a été créée.
# pour s'assurer que le code a fait ce qu'il était censé faire.
linelist_surv %>% 
  tabyl(outcome, event)
```

Maintenant, nous croisons la nouvelle variable `age_cat_small` et l'ancienne colonne `age_cat` pour nous assurer que les affectations ont été bien faites.  
...
* le nombre de personnes qui étaient à risque de développer l'événement (les personnes qui n'ont pas encore eu l'événement ou qui n'ont pas été censurées : `n.risk`)  
...
* finalement, l'erreur type et l'intervalle de confiance pour cette probabilité sont dérivés et affichés.  

Nous ajustons les estimations de la KM en utilisant la formule où l'objet "survobj", qui était un objet `Surv`, est la variable de réponse. "~ 1" précise que nous exécutons le modèle pour la survie globale.  
...
# imprimer son résumé pour plus de détails
summary(linelistsurv_fit)

```


En utilisant `summary()`, nous pouvons ajouter l'option `times` pour spécifier certaines heures auxquelles nous voulons voir les informations de survie. 
...
Nous pouvons également utiliser la fonction `print()`. L'argument `print.rmean = TRUE` permet d'obtenir le temps de survie moyen et son erreur type (SE).
...
```{r, mean_survtime}
# Imprimer l'objet linelistsurv_fit avec le temps de survie moyen et son SE. 
print(linelistsurv_fit, print.rmean = TRUE)

```


<span style="color : darkgreen ;">**_ASTUCE:_** Nous pouvons créer l'objet *surv* directement dans la fonction `survfit()` et sauvegarder une ligne de code. Cela ressemblera alors à : `linelistsurv_quick <- survfit(Surv(futime, event) ~ 1, data=linelist_surv)`.</span>
...
Parmi ces éléments, il y en a un important : `cumhaz`, qui est un vecteur numérique. Il pourrait être tracé pour permettre de montrer l'**_risque cumulatif_**, l'**_risque_** étant le **_taux instantané d'occurrence de l'événement_** (voir références).
...
Nous pouvons rapidement vérifier le temps de suivi minimum et maximum sur la courbe.  
...
L'intervalle de confiance des estimations de survie KM est également tracée par défaut et peut être éliminée en ajoutant l'option `conf.int = FALSE` à la commande `plot()`.
...
# Dessinez une courbe supplémentaire au graphique précédent
lines(
  linelistsurv_fit,
  lty = 3, # utiliser un type de ligne différent pour plus de clarté
  fun = "event", # dessiner les événements cumulés au lieu de la survie 
  mark.time = FALSE,
  conf.int = FALSE
  )
...
Le test du log rank est un test populaire qui compare l'ensemble de l'expérience de survie entre deux ou plusieurs groupes *indépendants* et peut être considéré comme un test permettant de savoir si les courbes de survie sont identiques (se chevauchent) ou non (hypothèse nulle d'aucune différence de survie entre les groupes). La fonction `survdiff()` du paquet **survival** permet d'exécuter le test log-rank lorsque l'on spécifie `rho = 0` (ce qui est le cas par défaut). Le résultat du test donne une statistique du khi carré ainsi qu'une valeur p puisque la statistique de log-rank est approximativement distribuée comme une statistique de test de khi-carré.
...
# créer le nouvel objet survfit par sexe
...
<span style="color : orange ;">**_ATTENTION:_** Les fonctions **survminer** exigent que vous spécifiiez l'objet de survie *et* que vous spécifiiez à nouveau les données utilisées pour ajuster l'objet de survie. N'oubliez pas de le faire pour éviter les messages d'erreur non spécifiques. </span>

```{r, warning=F, message=F}

survminer::ggsurvplot(
    linelistsurv_fit_sex, 
    data = linelist_surv, # spécifier à nouveau les données utilisées pour ajuster linelistsurv_fit_sex 
    conf.int = FALSE, # ne pas montrer l'intervalle de confiance des estimations KM
    surv.scale = "percent", # présenter les probabilités sur l'axe des ordonnées en %.
    break.time.by = 10, # présenter l'axe du temps avec un incrément de 10 jours
    xlab = "Jours de suivi",
    ylab = "Probabilité de survie",
    pval = T, # imprimer la valeur p du test de log-rank 
    pval.coord = c(40,.91), # imprimer la valeur p à ces coordonnées du graphique
    risk.table = T, # imprimer le tableau des risques en bas de page 
    legend.title = "Gender", # légende des caractéristiques
    legend.labs = c("Female", "Male"),
    font.legend = 10, 
    palette = "Dark2", # spécifier la palette de couleurs 
    surv.median.line = "hv", # dessiner des lignes horizontales et verticales sur les médianes de survie
    ggtheme = theme_light() # simplifier le fond du graphique
)
...
Nous pouvons d'abord ajuster un modèle pour évaluer l'effet de l'âge et du sexe sur la survie. En imprimant simplement le modèle, nous avons les informations sur :
  ...
  + leur erreur type `se(coef)`,
  + la cote z : combien d'erreurs type le coefficient estimé est-il éloigné de 0,
...
<span style="color : black ;">**_NOTE:_** Un deuxième argument appelé *method* peut être spécifié lors du calcul du modèle de cox, qui détermine comment les liens sont traités. Le *défaut* est "efron", et les autres options sont "breslow" et "exact".</span>
...
### Graphiques en forêt {.unnumbered}

Nous pouvons ensuite visualiser les résultats du modèle de cox en utilisant les graphiques en forêt pratiques avec la fonction `ggforest()` du paquet **survminer**.
...
L'analyse des covariables dépendantes du temps dans R nécessite la configuration d'un jeu de données spécial. Si cela vous intéresse, consultez l'article plus détaillé de l'auteur du paquet **survival** [Using Time Dependent Covariates and Time Dependent Coefficients in the Cox Model](https://cran.r-project.org/web/packages/survival/vignettes/timedep.pdf).
...
#### Élargir les lignes de patients {.unnumbered} 
...
Comme vous pouvez le constater à partir du graphique en forêt, de l'intervalle de confiance et de la valeur p, il ne semble pas y avoir de forte association entre le décès et la maladie aiguë du greffon contre l'hôte dans le contexte de notre modèle simple. 
