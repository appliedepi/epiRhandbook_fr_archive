...
Pour expliquer le nettoyage des données, cette page commence par l'importation d'un jeu de données brutes  de cas denommé linelist, et procède étape par étape dans le processus de nettoyage. Dans le code R, cela se manifeste par une chaîne "pipe", qui fait référence à l'opérateur "pipe" ` %>%` qui fait passer un jeu de données d'une opération à l'autre.  
...
`across()`|transformer plusieurs colonnes en meme temps|**dplyr** 
Fonctions **tidyselect**|utiliser la logique pour sélectionner des colonnes|**tidyselect**.   
`filter()`|conservation de certaines lignes|**dplyr** 
`distinct()`|déduplication des lignes|**dplyr***. 
`rowwise()`|opérations par/dans chaque ligne|**dplyr**  
...
`clean_variable_spelling()`|re-coder/nettoyer des valeurs en utilisant un dictionnaire de données|**linelist**
...
Vous pouvez rencontrer une methode alternative de gestion de données à partir du package  R **data.table** avec des opérateurs comme `:=` et l'utilisation fréquente de crochets `[ ]`. Cette approche et cette syntaxe sont brièvement expliquées dans la page [Data Table].  
...
<!-- ======================================================= -->
<!-- ======================================================= -->
## Nettoyage methodologie

**Cette page passe en revue les étapes importantes de nettoyage, en les ajoutant séquentiellement à une chaîne de nettoyage.**

Dans l'analyse épidémiologique et le traitement des données, les étapes de nettoyage sont souvent effectuées de manière séquentielle, reliées entre elles. En R, cela se manifeste souvent sous la forme d'un "pipeline" de nettoyage, où *le jeu de données brutes passé ou "pipé" d'une étape de nettoyage à une autre*.  
...
Ici, nous importons le fichier Excel "brut" de cas linelist en utilisant la fonction `import()` du package **rio**. Cet dernier  gère de manière flexible de nombreux types de fichiers (par exemple, .xlsx, .csv, .tsv, .rds). Consultez la page [Importation et exportation] pour plus d'informations et des conseils sur les situations inhabituelles (par exemple, sauter des lignes, définir des valeurs manquantes, importer des feuilles Google, etc.)  

Si vous voulez continuer, <a href='https://github.com/epirhandbook/Epi_R_handbook/raw/master/data/case_linelists/linelist_raw.xlsx' class='download-button'>cliquer pour telecharger le jeu de données  "brut" linelist</a> (comme fichier .xlsx ).  

Si votre jeu de données est important et prend beaucoup de temps pour  l'importation, il peut être utile que la commande d'importation soit séparée de la chaîne de pipe et que la donnée "brute" soit enregistré dans un fichier distinct. Cela permet également de comparer facilement les versions originales et nettoyées.  
...
D'autres logiciels statistiques tels que SAS et STATA utilisent des *"labels "* qui  representent un format  plus longues des noms de colonnes plus courts existants. Bien que R offre la possibilité d'ajouter des étiquettes de colonne aux données, cela n'est pas mis en avant en pratique. Pour rendre les noms de colonne "facilement affichage" pour les figures, on ajuste généralement leur affichage dans les commandes de representation graphique qui créent les sorties (par exemple, les titres des axes ou des légendes d'une graphique, ou les en-têtes de colonne dans un tableau imprimé - voir la section [scales section of the ggplot tips page](#ggplot_tips_scales) et les pages [Tableaux pour la présentation]). Si vous souhaitez attribuer des étiquettes de colonne dans les données, lisez la suite en ligne [ici](https://cran.r-project.org/web/packages/expss/vignettes/labels-support.html) et [ici](https://cran.r-project.org/web/packages/labelled/vignettes/intro_labelled.html).  
...
* Nomenclature de style similaire (par exemple, toutes les colonnes de date nommées comme **date_**apparition, **date_**rapport, **date_**mort...)  

Les noms des colonnes de `linelist_raw` sont affiché ci-dessous en utilisant `names()` de **base** R. On peut voir qu'initialement :  

* Certains noms contiennent des espaces (par exemple `date d'infection`)  
* Des patterns de noms différents sont utilisés pour les dates (`date onset` vs. `infection date`).  
* Il doit y avoir un *même nom* attribué  pour les deux dernières colonnes du fichier .xlsx. Nous le savons parce que le nom de deux colonnes fusionnées ("merged_header") a été attribué par R à la première colonne, et que la deuxième colonne s'est vue attribuer un nom fictif "...28" (puisqu'elle était alors vide et qu'il s'agit de la 28ème colonne).  
...
* Convertit tous les noms pour qu'ils ne soient composés que des undercores, de chiffres et de lettres.  
...
* Voici une [vignette] en ligne (https://cran.r-project.org/web/packages/janitor/vignettes/janitor.html#cleaning)  

Ci-dessous, le pipeline de nettoyage commence par utiliser `clean_names()` sur la donnée brute linelist.  

```{r clean_names}
# pipe le jeu de donnée brut à travers  clean_names(), assignons le resultat à "linelist"  
...
Si votre dataframe est déjà *groupée* (voir la page sur [Grouping data]), `mutate()` peut se comporter différemment que si la base de données n'est pas groupée. Toutes les fonctions de résumé, comme `mean()`, `median()`, `max()`, etc. seront calculées par groupe, et non par toutes les lignes.     
...
linelist <- linelist %>% 
  mutate(age_years = case_when(
            ...
            age_unit == "months" ~ age/12,    # si age est donnée en mois
            is.na(age_unit)      ~ age,       # Si age unit est à une valeur maquante remplacons par l'age
            TRUE                 ~ NA_real_)) # toute valeur non prise en compte par ces conditions seront consideres commme valeur manquante 
```


Lorsque chaque ligne des données est évaluée, les critères sont appliqués/évalués dans l'ordre où les instructions "case_when()`" sont écrites - de haut en bas. Si le premier critère est évalué à `TRUE` pour une ligne donnée, la valeur CD est attribuée, et les autres critères ne sont même pas testés pour cette ligne. Il est donc préférable d'écrire les critères les plus restrictifs en premier, et les plus généraux en dernier.  

Dans le même ordre d'idées, dans votre dernière ligne de code, placez `TRUE` sur le côté gauche, ce qui permettra de capturer toute ligne qui ne répond à aucun des critères précédents. Le côté droit de cette déclaration pourrait se voir attribuer une valeur comme "vérifiez-moi !" ou manquante.  


<span style="color: red;">**_DANGER:_** **Les valeurs du côté droit doivent toutes être de la même classe** - soit numérique, caractère, date, logique, etc. Pour attribuer des valeurs manquantes (`NA`), Il est dnas certaine situation important d'utiliser  des variantes spéciales de `NA` telles que `NA_caracter_`, `NA_real_` (pour les numériques ou POSIX), et `as.Date(NA)`. Pour en savoir plus, lisez [Working with dates].</span>  
...
Utilisez le package **linelist** et sa fonction `clean_variable_spelling()` pour nettoyer un dataframe avec un *dictionnaire de nettoyage*. **linelist** est un package développé par [RECON] (https://github.com/reconhub/linelist) - le R Epidemics Consortium.  
...
2) Importez le fichier du dictionnaire dans R. Cet exemple peut être téléchargé via les instructions de la page [Télécharger le manuel et les données].  
...
3) Passez le jeu de donnée brute linelist  à `clean_variable_spelling()`, en spécifiant à `wordlists = ` le dataframe du dictionnaire de nettoyage. L'argument `spelling_vars = ` peut être utilisé pour spécifier quelle colonne du dictionnaire fait référence aux colonnes (3ème par défaut), ou peut être defini comme `NULL` pour que le dictionnaire s'applique à toutes les colonnes de caractères et de facteurs. Notez que cette fonction peut prendre beaucoup de temps à s'exécuter.  

```{r}
linelist <- linelist %>% 
  linelist::clean_variable_spelling(
    wordlists = cleaning_dict,
    spelling_vars = "col",        # dict colonne contenant les noms de colonnes , par defaut  à la 3éme dans  dict
  )
```
...
**Ci dessous quelques nouveaux colonneset des transformations operes sur les colonnes existantes sont implementés dans la chaine pipé.**  
...
Pour une manière rapide de faire des pauses et de labelliser des vecteurs, utilisez quelque chose comme ci-dessous. Voir la page [R basics] pour les références sur `seq()` et `rep()`.  
...
*(Voir la page [Travailler avec des dates] pour plus d'informations sur le travail avec des dates et le paquet **lubridate**)*.
...
Si vous voulez effectuer un calcul dans une ligne, vous pouvez utiliser `rowwise()` de **dplyr**. Voir cette vignette en ligne sur [row-wise calculations] (https://cran.r-project.org/web/packages/dplyr/vignettes/rowwise.html).  
Par exemple, ce code applique `rowwise()` et crée une nouvelle colonne qui additionne le nombre de colonnes de symptômes spécifiées qui ont la valeur "yes", pour chaque ligne de la linelist. Les colonnes sont spécifiées dans `sum()` par leur nom dans un vecteur `c()`. `rowwise()` est essentiellement un type spécial de `group_by()`, il est donc préférable d'utiliser `ungroup()` lorsque vous avez terminé (page sur [Grouping data]).  
...
Placez les critères de spécification des colonnes dans la fonction **dplyr** `c_across()`. Ceci parce que `c_across` ([documentation](https://dplyr.tidyverse.org/reference/c_across.html)) est conçue pour fonctionner avec `rowwise()` spécifiquement. Par exemple, le code suivant :  
...
Listez simplement les colonnes dans l'ordre où elles doivent être triées. Spécifiez `.by_group = TRUE` si vous voulez que le tri se fasse d'abord par tout *groupement* appliqué aux données (voir la page sur [Grouping data]).  
