...
**Une jointure à gauche ou droite est une opération très couramment utilisée pour ajouter des informations à un cadre de données**, en particulier dans les analyses épidémiologiques. Les nouvelles informations sont ajoutées uniquement aux lignes qui existaient déjà dans le dataframe de "référence".  
...
* Par défaut, chaque ligne de l'un des deux cadres de données est comparée à une ligne au maximum de l'autre cadre de données. Si vous voulez voir toutes les correspondances évaluées, choisissez `dedupe.matches = FALSE`. La dé-duplication est faite en utilisant la technique de programmation linéaire de Winkler. 
...
Le dataframe `my_matches` contient deux colonnes contenant les numéros de lignes/indices (aussi appelés "rownames") de `cases` ("inds.a") et de `results` ("inds.b"), représentant les meilleures correspondances. Si un numéro de ligne d'un cadre de données est manquant, alors aucune correspondance n'a été trouvée dans l'autre cadre de données au seuil de correspondance spécifié.    
