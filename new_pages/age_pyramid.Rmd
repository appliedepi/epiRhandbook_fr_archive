...
Lors de l'utilisation du package **agepyramid**, si la colonne `split_by` est binaire (par exemple homme/femme, ou oui/non), le résultat apparaîtra comme une pyramide. Cependant, s'il y a plus de deux valeurs dans la colonne `split_by` (sans compter NA), la pyramide apparaîtra comme un diagramme à barres à facettes avec des barres grises dans le "fond" indiquant la plage des données non facettées pour ce groupe d'âge. Dans ce cas, les valeurs de `split_by =` apparaîtront comme des libellés en haut de chaque panneau de facettes. Par exemple, voici ce qui se passe si la colonne `hospital` est attribuée à `split_by =`.
...
Par défaut, les barres affichent les nombres (et non les pourcentages), une ligne en pointillés au milieu de chaque groupe est affichée, et les couleurs sont vertes/violettes. Chacun de ces paramètres peut être ajusté, comme indiqué ci-dessous :
...
Pour la démonstration, nous agrégeons les données de la liste de cas en nombre de cas par catégorie d'âge et par sexe, dans un format "large". Cela fera comme si vos données étaient aggrégées dès le départ. Pour en savoir plus sur le [Regroupement des données]{#grouping} et le [Pivotage des données]{#pivoting}, consultez leurs pages respectives.
...
Il y a beaucoup de choses que vous pouvez changer/ajoutez à cette version simple, y compris :
...
Importez et visualisez les données de population (voir la page [Télécharger le manuel et les données]{#data-used}) :
...
Combinez les données de population et de cas à l'aide de la fonction **dplyr** `bind_rows()` :
...
On peut maintenant faire la même chose pour les données des cas. Now implement the same for the case linelist. C'est légèrement différent car elle commence par les lignes de cas et non par des totaux.
...
Importez les données (voir la page [Télécharger le manuel et les données]{#data-used} si vous le souhaitez).
