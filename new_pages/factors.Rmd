...
Ce morceau de code montre le chargement des packages nécessaires aux analyses. Dans ce manuel, nous mettons l'accent sur la fonction `p_load()` du package **pacman**, qui installe le package si nécessaire *et* le charge pour l'utiliser. Vous pouvez également charger les packages installés avec la fonction `library()` de **base** de R. Voir la page sur [R basics](#rbasics) pour plus d'informations sur les packages R.  
...
```{r, eval=F}
# importer ton jeu de données
linelist <- import("linelist_cleaned.rds")
```


### Nouvelle variable qualitative {#fct_newcat .unnumbered}  
...
Notez que si vous convertissez une colonne numérique en classe facteur, vous ne serez pas en mesure de calculer des statistiques numériques sur celle-ci.  

#### Création de  colonnes {.unnumbered}  

Nous utilisons la colonne existante `days_onset_hosp` (jours entre l'apparition des symptômes et l'admission à l'hôpital) et créons une nouvelle colonne `delay_cat` en classant chaque ligne dans l'une de plusieurs catégories. Nous faisons cela avec la fonction `case_when()` du package **dplyr**, qui applique séquentiellement des critères logiques (côté droit) à chaque ligne et renvoie la valeur correspondante côté gauche pour la nouvelle colonne `delay_cat`. Vous trouverez plus d'informations sur la fonction `case_when()` dans [Nettoyage des données et des fonctions de base](#cleaning_data).
...
Telle que créée avec `case_when()`, la nouvelle colonne `delay_cat` est une colonne catégorielle de la classe Character - *pas* encore un facteur. Ainsi, dans un tableau de fréquences, nous voyons que les valeurs uniques apparaissent dans un ordre alphanumérique par défaut - un ordre qui n'a pas beaucoup de sens intuitif :  

```{r}
table(linelist$delay_cat, useNA = "always")
```

De même, si nous réalisons un diagramme en barres, les valeurs apparaissent également dans cet ordre sur l'axe des x (voir la page sur [les bases de ggplot](#ggplot_basics) pour en savoir plus sur **ggplot2** - le package de visualisation le plus courant dans R).  
...
Pour convertir une colonne de caractères ou de chiffres en facteur, vous pouvez utiliser n'importe quelle fonction du package **forcats** (plusieurs sont détaillées [ci-dessous](#fct_adjust)). Elles convertiront en variable de classe facteur et effectueront ou permettront un certain ordre des niveaux - par exemple, l'utilisation de la fonction `fct_relevel()` vous permet de spécifier manuellement l'ordre des niveaux. Quant à la fonction `as_factor()`, elle convertit simplement la classe sans autres capacités.  

La fonction `factor()` de R **base**  convertit une colonne en facteur et vous permet de spécifier manuellement l'ordre des niveaux, comme un vecteur de caractères à son argument `levels = `.  

Ci-dessous, nous utilisons les fonctions `mutate()` et `fct_relevel()` pour convertir la colonne `delay_cat` de la classe caractère à la classe facteur. La colonne `delay_cat` est créée dans la section [Préparation](#fct_newcat) ci-dessus. 
...
Les niveaux ont un *ordre*, qui peut être affiché avec la fonction `levels()` appartenant aux fonctions de **base** de R, ou alternativement visualisé dans un tableau de comptage via `table()` du package **base** de R ou `tabyl()` de **janitor**. Par défaut, l'ordre des niveaux sera alpha-numérique, comme auparavant. Notez que `NA` n'est pas un niveau de facteur.  
...
Maintenant, l'ordre des variables sur le graphique a aussi un sens plus intuitif.  
...
Si vous devez ajouter des niveaux à un facteur, vous pouvez le faire avec `fct_expand()`. Il suffit d'écrire le nom de la colonne suivi des nouveaux niveaux (séparés par des virgules). En affichant les niveaux dans un tableau, nous pouvons voir les nouveaux niveaux et le nombre de zéros. Vous pouvez utiliser la fonction `table()` du package **base** de R, ou `tabyl()` de **janitor** :  
...
Cette fonction est utilisée pour ordonner manuellement les niveaux des facteurs. Si elle est utilisée sur une colonne sans facteur, la colonne sera d'abord convertie en facteur.  
...
* Un niveau et son emplacement corrigé en utilisant l'argument `after = `.  
...
Si vous voulez seulement déplacer un niveau, vous pouvez le spécifier uniquement à l'aide de la fonction `fct_relevel()` et donner un nombre à l'argument `after = ` pour indiquer dans l'ordre dans lequel il doit être. Par exemple, la commande ci-dessous déplace "<2 jours" en deuxième position : 
...
Notez que le titre par défaut de l'axe des x est maintenant assez compliqué - vous pouvez changer ce titre avec l'argument `labs()` du package **ggplot2**.  
...
Il est assez fréquent que vous vouliez inverser l'ordre des niveaux. Vous pouvez le faire en utilisant simplement le facteur avec la fonction `fct_rev()`.  

Notez que si vous voulez inverser *seulement* la légende du graphique mais pas les niveaux réels du facteur, vous pouvez le faire avec la fonction `guides()` (voir la page [Astuces avec ggplot](#ggplot_tips))).  
...
Pour ordonner par fréquence de manière à ce que la valeur puisse apparaître dans les données, utilisez la fonction `fct_infreq()`. Toute valeur manquante (`NA`) sera automatiquement incluse à la fin, à moins qu'elle ne soit convertie en un niveau (voir [cette section](#fct_missing)). Vous pouvez inverser l'ordre en utilisant à nouveau la fonction `fct_rev()`.  
...
Utilisez la fonction `fct_inorder()` pour définir l'ordre des niveaux afin de correspondre à l'ordre d'apparition dans les données, en commençant par la première ligne. Cela peut être utile si vous avez d'abord soigneusement arrangé à l'aide de la fonction `arrange()` les données dans le jeu de données, et ensuite l'utiliser pour définir l'ordre des facteurs.  
...
Dans le premier exemple ci-dessous, l'ordre par défaut alpha-numérique est utilisé. Vous pouvez voir que les hauteurs des box plots sont mélangées et ne sont pas dans un ordre particulier. Dans le deuxième exemple, la colonne `delay_cat` (qui est projeté sur l'axe des x) a été intégrée dans la fonction `fct_reorder()`, la colonne `ct_blood` est donnée comme deuxième argument, et "median" est donné comme troisième argument (vous pourriez aussi utiliser "max", "mean", "min", etc). Ainsi, l'ordre des niveaux de `delay_cat` reflètera maintenant les valeurs médianes croissantes de la valeur médiane de CT de chaque groupe `delay_cat`. Ceci est reflété dans le deuxième graphique - les box plots ont été réarrangés pour être ascendants. Notez comment `NA` (manquant) apparaîtra à la fin, à moins d'être converti en un niveau explicite.  
...
Notez que dans l'exemple ci-dessus, aucune étape n'est nécessaire avant de faire appel à la fonction `ggplot()` - le regroupement et les calculs sont tous effectués en interne par la fonction ggplot.  
...
Vous pouvez ajuster l'affichage des niveaux manuellement avec `fct_recode()`. C'est comme la fonction `recode()` du package **dplyr**  (voir [Nettoyage des données et fonctions de base](#cleaning_data)), mais elle permet la création de nouveaux niveaux de facteurs. Si vous utilisez uniquement la fonction `recode()` sur un facteur, les nouvelles valeurs recodées seront rejetées à moins qu'elles n'aient déjà été définies comme des niveaux admissibles. 
...
Les nouveaux niveaux sont créés à l'aide de la syntaxe `fct_recode(colonne, "nouveau" = "ancien", "nouveau" = "ancien", "nouveau" = "ancien")` et affichés :  
...
Ici, ils sont combinés manuellement à partir de la fonction `fct_recode()`. Notez qu'aucune erreur n'est soulevée lors de la création d'un nouveau niveau "Moins de 5 jours".  
...
Pour "regrouper" plusieurs niveaux de basse fréquence dans un groupe "Autre", faites l'une des choses suivantes :  
...
Si vous préparez de nombreuses figures (par exemple, pour plusieurs juridictions ou districts sanitaires), vous voudrez que les légendes et les tableaux apparaissent de manière identique, même si les niveaux d'achèvement ou de composition des données varient.  
...
Tant le package **base** de R, la fonction `table()` ainsi que la fonction`tabyl()` du package **janitor** montreront tous les niveaux de facteurs (même les niveaux non utilisés).  

Si vous utilisez les fonction `count()` ou `summarise()` de **dplyr** pour faire une tableau, ajoutez l'argument `.drop = FALSE` pour inclure les comptes pour tous les niveaux de facteurs, même ceux qui ne sont pas utilisés.  
...
1) 1) *Pour un contrôle précis de l'affichage*, convertissez la colonne **lubridate** des épihebdomadaires (AAAA-MM-JJ) au format d'affichage souhaité (AAAA-WWW) *dans le cadre de données lui-même*, puis convertissez-la en classe Factor.  

Tout d'abord, utilisez `format()` de R **base** pour convertir l'affichage de la date de YYYY-MM-DD en YYYY-Www (voir la page [Manipuler les dates](#working_dates)). Dans ce processus, la classe sera convertie en caractère. Ensuite, convertissez le caractère en classe Factor avec `factor()`.  
