...
# Les bases de ggplot basics {#ggplot_basics}
...
Dans cette page, nous allons couvrir les principes fondamentaux de la visualisation avec **ggplot2**. Voir la page [ggplot tips](#ggplot_tips) pour des suggestions et des techniques avancées pour que vos graphiques soient vraiment esthétiques.  
...
```{r}
pacman::p_load(
  tidyverse,      # comprend ggplot2 et d'autres extensions de  data management
...
# afficher le jeu de donnees de la liste lineaire comme ue table
...
* Nettoyer certaines colonnes de manière à ce que leurs valeurs (qui étaient codées de façon à être maniables) avec des caractères spéciaux comme des "underscores", etc. soient transformées en texte normal ou en majuscules (voir [Caractères et chaînes de caractères](#character_strings)).  
...
linelist <- linelist %>%
  mutate(
    ...
    outcome_disp = replace_na(outcome, "Unknown")          # remplacer les valeurs NA de la variable "outcome" par "Unknown".
  )
```
...
Par exemple, supposons que nous voulons visualiser des données qui sont dans un format "large", comme pour chaque cas dans la `linelist` et leurs symptômes. Ci-dessous, nous créons une mini-liste appelée `symptoms_data` qui ne contient que les colonnes `case_id` et les différentes variables des symptômes. 
...
Voici à quoi ressemblent les 50 premières lignes de cette mini-liste linéaire - voyez comment elles sont présentées en format "large" avec chaque symptôme comme colonne : 
...
Si nous voulions représenter graphiquement le nombre de cas présentant des symptômes spécifiques, nous sommes quelque part limités par le fait que chaque symptôme est une colonne spécifique. Cependant, nous pouvons *restructurer* ("pivoter") les colonnes de symptômes dans un format plus long comme ceci :

```{r, }
symptoms_data_long <- symptoms_data %>%    # commencer avec la mini-liste linéaire appelée symptoms_data
  ...
  pivot_longer(
    ...
    values_to = "symptom_is_present") %>%  # assigner un nom à la nouvelle colonne qui va contenir les valeurs des différents symptômes regroupés (ici yes/no)
  
  mutate(symptom_is_present = replace_na(symptom_is_present, "unknown")) # convertir les valeurs NA en "unknown"
...
2. Ajouter des couches "geom" - ces fonctions sont des éléments de représentation graphique qui permettent de visualiser les données comme des formes géométriques (*geoms*), par exemple comme un graphique à barres, un graphique linéaire, un graphique en nuage de points, un histogramme (ou une combinaison des différentes formes!). Ces fonctions commencent toutes par le préfixe `geom_`.  
...
Un exemple simple de l'ossature d'un code permettant de dessiner un graphique avec **ggplot2** est le suivant. Nous allons expliquer chaque composante dans les sections ci-dessous. 

```{r, eval=F}
# représenter les données dans my_data comme des points coloriés en rouge
ggplot(data = my_data)+                   # utiliser la base "my_data"
  geom_point(                             # ajouter une couche de points (dots)
    mapping = aes(x = col1, y = col2),    # préciser quelle donnée de my_data nous voulons représenter sous forme de points en donnant les coordonnées précises des points pour chaque axe
    color = "red")+                       # autres specifications pour le geom
  labs()+                                 # ici on ajute les titres, nom des axes, etc.
  theme()                                 # ici on ajuste  les couleurs, police, taille etc pour les éléments du graphique ne dépendant pas des données(axes, titres, etc.) 
```
...
* Les boîtes à moustaches - `geom_boxplot()`.  
...
On parle ainsi de "mappage" qui n'est dans ce cadre rien d'autre qu'une mise en relation entre un attribut graphique du "geom" et une variable du jeu de données utilisé pour faire la représentation graphique.

Ce "mappage" (correspondance/assignation) se fait avec l'argument `mapping = `. Les "mappages" que nous fournissons à l'argument `mapping` doivent être enveloppés dans la fonction `aes()`, donc nous écrirons quelque chose comme `mapping = aes(x = col1, y = col2)`, comme montré ci-dessous.

Ci-dessous, dans la commande `ggplot()`, les données sont définies comme la liste linéaire des cas `linelist`. Dans l'argument `mapping = aes()`, la colonne `age` est mise en correspondance avec l'axe des x, et la colonne `wt_kg` est mise en correspondance avec l'axe des y.  
...
Dans le jargon de ggplot, l'"esthétique" d'un graphique a une signification assez spécifique. Il s'agit d'une propriété visuelle des *données représentées*. Notons que le terme "esthétique" fait ici référence aux *données de variables "mappées" dans les "geoms"/formes* - et non à l'affichage des éléments environnants du graphique (qu'on pourrait associer au mot "esthétique" en français courant) et qui eux, ne dépendent pas des données tel que les titres, les noms d'axe, la couleur de fond. Dans ggplot, ces éléments d'affichage non reliés aux données sont appelés "thèmes" et sont déterminés par une commande `theme()` (voir [cette section](#ggplot_basics_themes)).  
...
* `fill = ` La couleur intérieure (par exemple d'une barre ou d'un boxplot)  
* `color = ` La ligne extérieure d'une barre, d'un boxplot, etc., ou la couleur du point si on utilise `geom_point()`  
...
* `width = ` La largeur des colonnes du  diagramme en barres  
...
* Dans le premier exemple, l'instruction `mapping = aes()` se trouve dans la commande `ggplot()` et les axes sont associés aux variables d'âge et de poids de notre jeu de données. Les attributs du graphique, `color = `, `size = `, et `alpha = ` (pour déterminer la transparence) sont assignées à des valeurs statiques. Pour plus de clarté, ceci est fait dans la fonction `geom_point()`, car vous pouvez ajouter d'autres "geoms" par la suite qui prendraient des valeurs différentes pour leur esthétique d'affichage.  
* Dans le second exemple, l'histogramme ne nécessite que la mise en relation de la variable d'intérêt à l'axe des x . Les valeurs statiques de l'histogramme `binwidth = `, `color = `, `fill = ` (couleur de  l'intérieur des barres) et `alpha = ` sont à nouveau définies dans le "geom". 
...
Pour ce faire, nous devons associer l'attribut du graphique à un *nom de variable de notre jeu de données* (sans guillemets) ie le "mapper". Ceci doit donc être fait dans une fonction `mapping = aes()` (note : il y a plusieurs endroits dans le code où nous pouvons faire ces assignations (de mapping), comme discuté [ci-dessous](##ggplot_basics_map_loc)).  

Deux exemples sont présentés ci-dessous.  

* Dans le premier exemple, l'attribut `color = ` (de chaque point) est mappée à la variable `age` - et une échelle est apparue dans la légende ! Pour l'instant, notons simplement que l'échelle existe - nous montrerons comment la modifier dans les sections suivantes.  
* Dans le second exemple, deux nouveaux attributs sont également associées à des variables (`color = ` et `size = `), tandis que les attributs `shape = ` et `alpha = ` sont associés à des valeurs fixes en dehors de toute fonction `mapping = aes()`. 

```{r, out.width=c('50%', '50%'), fig.show='hold', warning=F, message=F}
# Diagramme de dispersion
ggplot(data = linelist,   # définir les données
       mapping = aes(     # mapper l'attribut aux valeurs de la colonne
         x = age,           # mapper l'axe des x  à la variable age            
         y = wt_kg,         # mapper l'axe des y à la variable poids
         color = age)
       )+     # mapper l'attribut color à la variable age
...
ggplot(data = linelist,   # définir les données
       mapping = aes(     # mapper les attributs aux variables
         x = age,           # mapper l'axe des x à la variable age            
         y = wt_kg,         # mapper l'axe des y à la variable poids
         color = age,       #mapper l'attribut color à la variable age
         size = age))+      # mapper l'attribut size (taille des points) à la variable age
  geom_point(             # afficher les données comme des points
...
Note : Les axes sont toujours assignées à des variables dans les données (pas à des valeurs statiques), et sont donc toujours mappées avec `mapping = aes()`.  


Il devient important de garder la trace des différentes couches du graphique et les attributs lorsque vous créez des graphiques plus complexes - par exemple des graphiques avec des "geoms" multiples. Dans l'exemple ci-dessous, l'attribut `size = ` est assigné deux fois - une fois pour `geom_point()` et une fois pour `geom_smooth()` - et les deux fois comme une valeur statique.  
...
Le mappage dans `mapping = aes()` peut être mentionné à plusieurs endroits dans les commandes du  ggplot et peut même être mentionné plus d'une fois. Cela peut être écrit dans la commande supérieure `ggplot()`, et/ou pour chaque "geom" individuel en dessous. Les possibilités comprennent :  

* Les affectations de mappage effectuées dans la commande supérieure `ggplot()` et qui seront héritées par défaut dans tous les "geom" inférieurs, comme c'est le cas pour `x = ` et `y = `. 
* Les affectations de mappage effectuées dans un "geom" et qui ne s'appliquent qu'à ce "geom".  

De même, l'argument `data = ` spécifié dans la commande supérieure `ggplot()` s'appliquera par défaut à tous les "geom" inférieurs. Toutefois on peut aussi spécifier des jeux de données différents pour chaque "geom" (mais c'est un plus complexe).  
...
#  Ces commandes produiront exactement le même tracé.
ggplot(data = linelist, mapping = aes(x = age))+
  geom_histogram()
...
Notez que selon le "geom", vous devrez utiliser différents arguments pour regrouper les données. Pour `geom_point()`, vous utiliserez probablement `color =`, `shape = ` ou `size = `. Alors que pour `geom_bar()`, vous utiliserez plus probablement `fill = `. Cela dépend simplement du "geom" et de l'attribut du graphique que vous voulez utiliser pour refléter les groupages.  

Pour votre information - la manière la plus basique de grouper les données est d'utiliser seulement l'argument `group = ` dans `mapping = aes()`. Cependant, cela ne changera pas les couleurs, le remplissage ou les formes. Elle ne créera pas non plus de légende. Pourtant, les données sont groupées, donc les affichages statistiques des données peuvent être affectés.  

Pour ajuster l'ordre des groupes dans un graphique, consultez la page [Trucs et Astuces avec ggplot ](#ggplot_tips) ou la page sur les [Facteurs](#factors). Vous trouverez de nombreux exemples de tracés groupés dans les sections ci-dessous sur la représentation des données continues et catégorielles.   
...
Le "faceting" est une fonctionnalité fournie avec **ggplot2**, de sorte que les légendes et les axes de chaque graphe d'un sous-groupe du jeu de données ("facet") sont automatiquement alignés. Il existe d'autres extensions ("packages") discutés dans la page [Trucs et Astuces avec ggplot](*ggplot_tips) qui sont utilisés pour combiner des graphiques complètement différents (ie qui cette fois ne sont pas les même graphique répétés pour chaque sous-groupe d'un même jeu de données)  en une seule figure. On peut citer **cowplot** et **patchwork**.  

Le "faceting" est effectué avec l'une des fonctions **ggplot2** suivantes :

  1. `facet_wrap()` Pour montrer un graphique différent pour chaque niveau d'une variable *simple*. Un exemple de ceci pourrait être de montrer une courbe d'épidémie différente pour chaque hôpital dans une région. Les "facets" sont ordonnées par ordre alphabétique, sauf si la variable est un facteur avec un autre ordre défini.  
...
Les "facets" peuvent rapidement contenir une quantité écrasante d'informations - il est bon de s'assurer que nous n'avons pas trop de modqlités pour chaque variable qualitative que nous choisissons de "facetter". Voici quelques exemples rapides avec le jeu de données sur le paludisme (voir [Télécharger le manuel et les données](#data_used)) qui contient les données sur le nombre de cas  de cas de paludisme quotidiens pour différentes structures de santé par groupe d'âge.  
...
Une des particularités de **ggplot2** est que nous pouvons définir un graphe (comme ci-dessus), puis lui ajouter des couches en commençant par son nom. Nous n'avons pas besoin de répéter toutes les commandes qui ont créé le graphe original ! 
...
* Spécifier le nom de l'objet plot, puis le chemin et le nom du fichier avec l'extension.  
  * Par exemple : `ggsave(my_plot, here("plots", "my_plot.png"))`  
* Exécutez la commande avec seulement un chemin de fichier, pour sauvegarder le dernier graphe qui a été imprimé.  
  * Par exemple : `ggsave(here("plots", "my_plot.png"))`.  
  
Vous pouvez exporter en png, pdf, jpeg, tiff, bmp, svg, ou plusieurs autres types de fichiers, en spécifiant l'extension du fichier dans le chemin du fichier.  

Vous pouvez également spécifier les arguments `width = `, `height = `, et `units = ` (soit "in", "cm", ou "mm"). Vous pouvez également spécifier `dpi = ` avec un nombre pour la résolution du graphe (par exemple 300). Consultez les détails de la fonction en entrant `?ggsave` ou en lisant la [documentation en ligne](https://ggplot2.tidyverse.org/reference/ggsave.html). 

Rappelez-vous que vous pouvez utiliser la syntaxe `here()` pour fournir le chemin du fichier souhaité. Voir la page [Importation et exportation](#import_export) pour plus d'informations. 


## Etiquettes du graphe

Vous voudrez certainement ajouter ou ajuster les étiquettes du graphique. Ceci est plus facile à faire avec la fonction `labs()` qui est ajoutée au graphe avec `+` tout comme les "goems" l'ont été.  
...
* `caption = ` La légende du graphique, en bas à droite par défaut.  
...
age_by_wt <- ggplot(
  ...
  mapping = aes(     # mapper les attributs aux valeurs des variables
         x = age,           # mapper l'axe des x à l'age            
         y = wt_kg,         # mapper l'axe des y au poids (weight)
         color = age))+     # mapper la coleur à l' age
  geom_point()+           # afficher les données comme des points
  labs(
...
* Ajustez chaque petit aspect de la parcelle individuellement dans `theme()`.  
...
<span style="color: black;">**_NOTE:_** Notez que tout micro-ajustement *supplémentaire* avec `theme()` doit être fait *après* l'utilisation d'une fonction thème prête à l'utilisation (sinon les ajustements ne seront pas pris en compte).</span>  
...
1. Dans `theme()` écrivez le nom de l'argument pour l'élément de l'intrigue que vous voulez modifier, comme `plot.title = `  
3. Fournissez une fonction `element_()` à l'argument  
  + Le plus souvent, utilisez `element_text()`, mais d'autres incluent `element_rect()` pour les couleurs d'arrière-plan du canevas, ou `element_blank()` pour supprimer les éléments du graphe  
4. A l'intérieur de la fonction `element_()`, écrivez des affectations d'arguments pour faire les ajustements poussés que vous désirez.  

Cette description était assez abstraite, voici donc quelques exemples.  

Le graphe ci-dessous a l'air assez stupide, mais il sert à vous montrer une variété de façons dont vous pouvez ajuster vos graphes.  

* Nous commençons avec le graphe `age_by_wt` défini juste au-dessus et ajoutons `theme_classic()`.  
* Pour des ajustements plus poussés, on ajoute `theme()` et on inclut un argument pour chaque élément du graphe à ajuster.  
...
```{r, , warning=F, message=F}
age_by_wt + 
  theme_classic()+                                 # pre-definir les ajustements ave un theme tout-prêt
  theme(
...
`axis.line = element_line()`       |Lignes des axes (colour, size, linetype: solid dashed dotted etc)
`strip.text = element_text()`      |Bande de texte des "facet" (colour, face, size, angle...)
...
<span style="color: darkgreen;">**_TIP:_** Si vous voulez supprimer un élément d'un graphe, vous pouvez aussi le faire via `theme()`. Passez simplement `element_blank()` en argument pour le faire disparaître complètement. Pour les légendes, préciser `legend.position = "none"`.</span>  
...
Veuillez consulter cette [section sur les gammes de couleurs de la page "Trucs et Astuces dans ggplot"](#ggplot_tips_colors).
...
* Les tracés pour une variable continue :  
  * **Histogramme**, un graphique classique pour présenter la distribution d'une variable continue. 
  **Graphique en boîte** (également appelé boîte à moustaches), pour montrer les 25ème, 50ème et 75ème percentiles, les extrémités de la distribution et les valeurs aberrantes ([limitations importantes](https://www.data-to-viz.com/caveat/boxplot.html)).  
  ...
  **Sina plot**, est une combinaison de jitter et de violin plot, où les points individuels sont montrés mais dans la forme symétrique de la distribution (via le "package" **ggforce**).  
...
Le code ci-dessous permet de générer des **histogrammes**, qui regroupent les données continues en plages et les affichent dans des barres adjacentes de hauteur variable. Ceci est fait en utilisant `geom_histogram()`. Voir la section ["Diagrammes en barres"](#ggplot_basics_bars) de la page ggplot basics pour comprendre la différence entre `geom_histogram()`, `geom_bar()`, et `geom_col()`.  
...
### Boîtes à moustaches {.unnumbered}
...
Nous vous rappelons ci-dessous les différentes composantes d'un boxplot :  
...
Dans la plupart des "geoms", nous créons un graphique par groupe en faisant correspondre un attribut comme `color = ` ou `fill = ` à une variable dans `aes()`. Cependant, pour les boxplot, nous pouvons le faire en assignant la variable de regroupement à l'axe non assigné (x ou y). Ci-dessous se trouve le code pour un boxplot de *toutes* les valeurs d'âge dans l'ensemble de données, et ensuite le code pour afficher un box plot pour chaque sexe (non manquant) dans l'ensemble du jeu de données. Notez que les valeurs `NA` (manquantes) apparaîtront comme un box plot séparé, sauf si elles sont supprimées. Dans cet exemple, nous avons également défini le "remplissage" de la colonne "outcome" (issue finale de chaque cas) afin que chaque diagramme soit d'une couleur différente - mais ce n'est pas nécessaire. 

```{r fig.show='hold', message=FALSE, warning=FALSE, out.width=c('50%', '50%')}
# A) boxplot d'ensemble
ggplot(data = linelist)+  
  geom_boxplot(mapping = aes(y = age))+   # uniquement axe y mappé (non axe des x)
  labs(title = "A) Overall boxplot")

# B) Boxplot par groupe
ggplot(data = linelist, mapping = aes(y = age, x = gender, fill = gender)) + 
  geom_boxplot()+                     
  theme(legend.position = "none")+   # supprimer legende (redondant)
...
### Graphes en violon, guigue, et "sina" {.unnumbered}

Ci-dessous, vous trouverez le code pour créer des diagrammes **violin** (`geom_violin`) et **jitter** (`geom_jitter`) pour montrer les distributions. Vous pouvez spécifier que le remplissage ou la couleur est également déterminé par les données, en insérant ces options dans `aes()`. 

```{r fig.show='hold', message=FALSE, warning=FALSE, out.width=c('50%', '50%')}


# A) Jitter par groupe
ggplot(data = linelist %>% drop_na(outcome),      # supprimer valeurs manquantes
       mapping = aes(y = age,                     # mapper variable continue
           x = outcome,                           # mapper la variable de regroupement
           color = outcome))+                     # mapper Couleur avec var outcome 
  geom_jitter()+                                  # Creer le graphe
...
ggplot(data = linelist %>% drop_na(outcome),       # supprimer valeurs manquantes
       mapping = aes(y = age,                      # mapper variable continue
           x = outcome,                            # mapper la variable de regroupement
           fill = outcome))+                       # mapper Couleur avec var outcome
  geom_violin()+                                   # Creer le graphe
...
ggplot(
  ...
  aes(y = age,           # mapper var numerique
      x = outcome)) +    # mapper variable de regroupement
  geom_violin(
    aes(fill = outcome), # remplissage (couleur de fond violon)
    color = "white",     # contour blanc
    alpha = 0.2)+        # transparence
  geom_sina(
    size=1,                # Changer la taille des guigues
    aes(color = outcome))+ # mapper  la couleur des points avec la var outcome
  scale_fill_manual(       # Definir des couleurs de remplissage (de fond) des violons en precisant quelle couleur prend chaque modalite de la var outcome
    values = c("Death" = "#bf5300", 
              "Recover" = "#11118c")) + 
  scale_color_manual(      # Definir des couleurs des points en precisant quelle couleur prend chaque modalite de la var outcome
    values = c("Death" = "#bf5300", 
              "Recover" = "#11118c")) + 
  theme_minimal() +                                # Supprimer arriere-plan gris
  theme(legend.position = "none") +                # Supprimer  legende non-necessaire
...
ggplot(data = linelist %>% drop_na(gender, outcome), #garder que le sexe/issue non manquant
       mapping = aes(y = wt_kg, x = age))+
  geom_point() +
...
Vous pouvez afficher trois variables continues en utilisant l'argument `fill = ` pour créer un *heat plot*. La couleur de chaque "cellule" reflétera la valeur de la troisième colonne de données continues. Voir la page [Astuces en ggplot](#ggplot_tips) et la page sur les [Heat plots] pour plus de détails et plusieurs exemples. 
...
## Représenter des données categorielles

Les données catégorielles peuvent être des valeurs de caractères, des valeurs logiques (VRAI/FAUX) ou des facteurs (voir la page [Facteurs](#factors)). 
...
```{r}
# Voir la classe de la variable hospital - on peut voir que c'est une variable caractere
...
# A) Outcome pour l'ensemble des cas
...
# B) Outcomes pour les cas par hopital
ggplot(linelist %>% drop_na(outcome)) + 
...
outcomes <- linelist %>% 
  ...
  group_by(outcome) %>% 
  count %>% 
  ungroup() %>% # Ungroup pour que les proportions soient sur le total
  mutate(proportion = n/sum(n)*100) # Caculer le pourcentage
  
outcomes #Voir la table
```


Le code ci-dessous utilise `geom_col` pour créer des diagrammes en barres simples afin de montrer la distribution de l'issue des cas Ebola. Avec `geom_col`, x et y doivent être spécifiés. Ici, x est la variable catégorique sur l'axe des x, et y est la colonne de proportions générée `proportion`. 


```{r, fig.height = 3, fig.width=4.5}
# Outcome pour l'ensemble des cas
ggplot(outcomes) + 
...
Pour montrer les répartitions par hôpital, il faudrait que notre tableau contienne plus d'informations et qu'il soit au format "long". Nous créons ce tableau avec les fréquences des catégories combinées `outcome` et `hospital` (voir la page [Grouping data](#grouping_data) pour des conseils sur le groupage). 

```{r, fig.height = 4, fig.width=6}
outcomes2 <- linelist %>% 
  drop_na(outcome) %>% 
  count(hospital, outcome) %>%  # compter les lignes par hospital et outcome
  group_by(hospital) %>%        # Grouper pour que les proportions soient sur le total de "hospital"
  mutate(proportion = n/sum(n)*100) # calculer proportions  de deces et de guerison au sein de chaque  "hospital" 

head(outcomes2) # Voir les premieres lignes de la table
```
...
Nous créons ensuite le ggplot avec quelques mises en forme supplémentaires :
  ...
  **Barres côte-à-côte** : Ajout d'un argument `position = "dodge"` pour que les barres pour les décès et la guérison soient présentées côte à côte plutôt qu'empilées. Notez que les barres empilées sont la valeur par défaut.
  * **Largeur de colonne** : Spécifié 'width', donc les colonnes sont deux fois moins larges que la largeur maximale possible.
  **Ordre des variable** : Inversez l'ordre des catégories sur l'axe des y de sorte que 'Autre' et 'Manquant' soient en bas, avec `scale_x_discrete(limits=rev)`. Notez que nous avons utilisé cette méthode plutôt que `scale_y_discrete` parce que l'hôpital est indiqué dans l'argument `x` de `aes()`, même si visuellement il est sur l'axe des ordonnées. Nous faisons cela parce que Ggplot semble présenter les catégories à l'envers, sauf si nous lui disons de ne pas le faire.  
  * **Autres détails** : Étiquettes/titres et couleurs ajoutés dans `labs` et `scale_fill_color` respectivement.
  
```{r, fig.height = 4, fig.width=8}

# Outcome pour l'ensemble des cas par hopital
ggplot(outcomes2) +  
  geom_col(
    mapping = aes(
      x = proportion,                 # mapper axe des x avec les  proportion pre-calculees
      y = fct_rev(hospital),          # inverse les categories de la variable donc missing/other en dernier
      fill = outcome),                # remplissage par outcome
    width = 0.5)+                    # barres moins larges (out of 1)
  theme_minimal() +                  # theme Minimal 
  theme(legend.position = "bottom")+
  labs(subtitle = "Number of recovered and dead Ebola cases, by hospital",
       fill = "Outcome",             # titre legend 
       y = "Count",                  # titre axe y 
       x = "Hospital of admission")+ # titre axe x
  scale_fill_manual(                 # preciser des couleurs manuellement
...
Si vous utilisez `geom_col()` avec des données de dates (par exemple une courbe epidemique à partir de données agrégées) - vous voudrez ajuster l'argument `width = ` pour supprimer les lignes de "gap" entre les barres. Si vous utilisez des données quotidiennes, réglez `width = 1`. Si vous utilisez des données hebdomadaires, `width = 7`. 
