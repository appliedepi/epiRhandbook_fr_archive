...
-   La page [Dashboards avec R Markdown](#dashboards) explique comment formater un rapport R Markdown en tant que tableau de bord.
...
- Si vous souhaitez générer une sortie PDF (un peu plus délicat), vous devrez installer **LaTeX**. Pour les utilisateurs de R Markdown qui n'ont pas installé LaTeX auparavant, nous vous recommandons d'installer [**TinyTeX** via ce lien](https://yihui.name/tinytex/). Vous pouvez utiliser les commandes suivantes :
...
Pour faire référence à des fichiers ailleurs, vous devrez soit utiliser le chemin complet du fichier, soit utiliser le "package" **here**. Le "package" **here** définit le répertoire de travail comme étant le dossier racine du projet R et est expliqué en détail dans les pages [Projets R]() et [Importation et exportation](#import_export) de ce manuel. Par exemple, pour importer un fichier appelé "data.csv" depuis le dossier `projectX`, le code serait `import(here("data.csv"))`.
...
Appelées "métadonnées YAML" ou simplement "YAML", elles se trouvent en haut du document R Markdown. Cette section du script indique à votre fichier Rmd le type de sortie à produire, les préférences de formatage et d'autres métadonnées telles que le titre du document, l'auteur et la date. Il existe d'autres utilisations qui ne sont pas mentionnées ici (mais auxquelles il est fait référence dans la section ["Produire une sortie"](#production)). <span style="color: black;">**_NOTE:_**  Notez que l'indentation est importante ; les tabulations ne sont pas acceptées, mais les espaces le sont.</span>
...
- Après le r, vous pouvez éventuellement écrire un "nom" de chunk -- ceux-ci ne sont pas nécessaires mais peuvent vous aider à organiser votre travail. Notez que si vous nommez vos morceaux, vous devez _**TOUJOURS**_ utiliser des noms uniques, sinon R se plaindra lorsque vous essaierez de compiler  le rendu.
...
<span style="color: black;">**_NOTE:_** Rappelez-vous, votre chemin de fichier pourrait être écrit en utilisant le "package". **here**</span>
...
<span style="color: orange;">**_CAUTION:_** Sachez que lors de l'affichage de la sortie HTML par onglets, la fonctionnalité de recherche Ctrl+f ne recherchera que les onglets "actifs", et non les onglets cachés.</span>
...
# ou bien sys.source("your-script.R", envir = knitr::knit_global())
```
...
<span style="color: orange;">**_CAUTION:_** Vous devez être conscient des différents *environnements* de R. Les objets créés dans un environnement ne seront pas nécessairement disponibles dans l'environnement utilisé par le Markdown R.</span>
...
#### Option 3 : Définir les paramètres à l'aide d'une interface utilisateur graphique {.non numéroté}
...
Lors de l'exécution du fichier (via le bouton "knit with parameters" ou par `render()`), la fenêtre pop-up aura des options déroulantes à sélectionner.
...
- Plotly (utilisé dans cette page du manuel et dans la page [Interative plots]).
- visNetwork (utilisé dans la page [Chaînes de transmission] de ce manuel)
- Leaflet (utilisé dans la page [Notions de base sur les SIG](#gis) de ce manuel)
...
La fonction `ggplotly()` de **plotly** est particulièrement facile à utiliser. Voir la page [Interactive plots].

## Ressources

Des informations supplémentaires peuvent être trouvées via :
