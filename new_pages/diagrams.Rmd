...
# Toutes les instructions sont dans une grande chaîne de caractères
# 'digraph' signifie 'graphique directionnel', puis le nom du graphique 
...
digraph surveillance_diagram { 
  ...
  {Primary Secondary Tertiairy} -> SC [label = 'déclaration des cas',
                                      fontcolor = darkgreen,
                                      couleur = darkgreen,
                                      style = dashed]
}
...
digraph surveillance_diagram {  
  ...
  sous-graphe cluster_EBD {
    EBS [label = 'Surveillance basée sur les événements (SBE)'] 
    'Social Media'
    Radio
  }
  
  sous-graphe cluster_CBS {
    CBS [label = 'Surveillance basée sur les communautés(SBC)']
    RECOs
  }
  ...
  Primary -> Secondary [label = 'transfert de cas',
                          fontcolor = red]
  Secondaire -> Tertiaire [label = 'transfert de cas',
                          fontcolor = red]
  
  HCF_active -> Active
  
  {Radio 'Social Media' -> EBS
...
data <- read.csv(text="event, groupe, début, fin, couleur
