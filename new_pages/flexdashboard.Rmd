...
Le script est un script R Markdown, et a donc les mêmes composants et la même organisation que ceux décrits dans la page sur les [Rapports avec R Markdown]. Nous allons brièvement les revoir et souligner les différences avec les autres formats de sortie R Markdown.  

### YAML {.unnumbered} (non numéroté)  

En haut du script se trouve l'en-tête "YAML". Il doit commencer par trois tirets `---` et doit se terminer par trois tirets `---`. Les paramètres YAML sont présentés par paires `key:value`. **L'indentation et le placement des deux points dans YAML sont importants** : les paires `key:value` sont séparées par des deux points (et non par des signes égaux !). 
...
Un script R Markdown peut contenir plusieurs "chunks" de code - il s'agit de zones du script dans lesquelles vous pouvez écrire du code R sur plusieurs lignes et qui fonctionnent comme des mini-scripts R. Les "chunks" de code sont créés à l'aide de trois lignes de code.  
...
En dehors d'un "chunk" de code R, vous pouvez écrire un texte narratif. Comme décrit dans la page [Reports with R Markdown](#rmarkdown), vous pouvez mettre du texte en italique en l'entourant d'un astérisque (*texte italique*), ou en gras en l'entourant de deux astérisques (**texte gras**). Rappelez-vous que les puces et les schémas de numérotation sont sensibles aux nouvelles lignes, à l'indentation et au fait de terminer une ligne par deux espaces.  

Vous pouvez également insérer du code R en ligne dans du texte, comme décrit à la page [Reports with R Markdown](#rmarkdown), en entourant le code de barres obliques inversées et en commençant la commande par "r" : `` ` 1+1` ``(voir l'exemple avec la date ci-dessus).  



### Titres {.unnumbered}  

Différents niveaux de titres sont établis avec différents nombres de symboles de hachage, comme décrit dans la page [Reports with R Markdown](#rmarkdown).  
...
```{r, out.width = c('100%'), out.height = c('100%'), echo=F}
knitr::include_graphics(ici::here("images", "flexdashboard_tables_script.png"))
```
...
Vous pouvez également passer un ggplot standard ou un autre objet de tracé à `ggplotly()` du paquet **plotly** (voir la page [Interactive plots]). Cela rendra votre graphique interactif, permettra au lecteur de "zoomer" et affichera en surimpression la valeur de chaque point de données (dans ce scénario, le nombre de cas par semaine et le groupe d'âge dans la courbe).  
...
* Leaflet (utilisé dans la page [GIS Basics] de ce manuel)  
...
```{r, out.width = c('100%'), out.height = c('100%'), echo=F}
knitr::include_graphics(ici::here("images", "flexdashboard_shiny_view.gif"))
```
