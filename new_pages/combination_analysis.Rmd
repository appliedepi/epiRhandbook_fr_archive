...
linelist_sym <- linelist %>% 
  mutate(fever = sample(c("yes", "no"), nrow(linelist), replace = T, prob = c(0.80, 0.20)),
         frissons = échantillon(c("oui", "non"), nrow(linelist), replace = T, prob = c(0.20, 0.80)),
         toux = échantillon(c("oui", "non"), nrow(linelist), replace = T, prob = c(0.9, 0.15)),
         douleurs = échantillon(c("oui", "non"), nrow(linelist), replace = T, prob = c(0.10, 0.90)),
         vomissements = échantillon(c("oui", "non"), nrow(linelist), replace = T))
...
# Créer le graphique
UpSetR::upset(
  select(linelist_sym_2, fièvre, frissons, toux, courbatures, vomi),
  ...
  point.size = 3.5,
  line.size = 2, 
  mainbar.y.label = "Combinaisons de symptômes",
...
pacman::p_load(
  ...
  UpSetR, # paquetage spécial pour les graphiques combinés
  ggupset) # paquetage spécial pour les tracés combinés
```
...
Pour s'aligner sur le format attendu par **ggupset**, nous convertissons les "yes" et "no" en nom de symptôme réel, en utilisant `case_when()` de **dplyr**. Si "non", nous mettons la valeur en blanc, donc les valeurs sont soit `NA` ou le symptôme.  
...
linelist_sym_1 <- linelist_sym %>% 
  ...
  mutate(
      ...
      fièvre == "oui" ~ "fièvre", # si l'ancienne valeur est "oui", la nouvelle valeur est "fièvre".
      TRUE ~ NA_caractère_), # si l'ancienne valeur est autre que "oui", la nouvelle valeur est NA
...
linelist_sym_1 <- linelist_sym_1 %>% 
  unite(col = "all_symptoms",
        c(fièvre, frissons, toux, courbatures, vomissements), 
        sep = " ; ",
        remove = TRUE,
        na.rm = TRUE) %>% 
  mutate(
...
``{r, warning=F, message=F}
ggplot(
  données = linelist_sym_1,
  mapping = aes(x = all_symptoms_list)) +
geom_bar() +
scale_x_upset(
  reverse = FALSE,
  n_intersections = 10,
  sets = c("fièvre", "frissons", "toux", "courbatures", "vomissements"))+
labos(
...
linelist_sym_2 <- linelist_sym %>% 
  ...
  mutate(
      ...
      fièvre == "oui" ~ 1, # si l'ancienne valeur est "oui", la nouvelle valeur est 1
...
# Créer le graphe
UpSetR::upset(
  select(linelist_sym_2, fièvre, frissons, toux, courbatures, vomissements),
  sets = c("fever", "chills", "cough", "aches", "vomit"),
  order.by = "freq",
  sets.bar.color = c("blue", "red", "yellow", "darkgreen", "orange"), # couleurs optionnelles
  empty.intersections = "on",
