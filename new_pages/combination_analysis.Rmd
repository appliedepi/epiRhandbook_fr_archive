...
linelist_sym_2 <- linelist_sym %>% 
  ...
  mutate(fever = case_when(fever == "yes" ~ 1, # si l'ancienne valeur est "oui", la nouvelle valeur est "fever"
...
linelist_sym_1 <- linelist_sym %>% 
  ...
  mutate(
    fever = case_when(
      fever == "yes" ~ "fever", # si l'ancienne valeur est "oui", la nouvelle valeur est "fever".
      TRUE           ~ NA_character_), # si l'ancienne valeur est autre que "oui", la nouvelle valeur est NA
...
linelist_sym_1 <- linelist_sym_1 %>% 
  unite(col = "all_symptoms",
        c(fever, chills, cough, aches, vomits), 
        sep = " ; ",
        remove = TRUE,
        na.rm = TRUE) %>% 
  mutate(
...
scale_x_upset(
  reverse = FALSE,
  n_intersections = 10,
  sets = c("fever", "chills", "cough", "aches", "vomits"))+
labos(
...
linelist_sym_2 <- linelist_sym %>% 
  ...
  mutate(
    fever = case_when(
      fever == "yes" ~ 1, # si l'ancienne valeur est "oui", la nouvelle valeur est 1
...
# Créer le graphe
UpSetR::upset(
  select(linelist_sym_2, fever, chills, cough, aches, vomits),
  sets = c("fever", "chills", "cough", "aches", "vomit"),
  order.by = "freq",
  sets.bar.color = c("blue", "red", "yellow", "darkgreen", "orange"), # couleurs optionnelles
  empty.intersections = "on",
