...
pacman::p_load(
  ...
  ici, # chemins de fichiers relatifs
...
ggplot(data = linelist %>% drop_na(outcome), # supprimer les valeurs manquantes
       mapping = aes(y = age, # Variable continue
           x = résultat, # variable de groupement
...
ggplot(données = linelist %>% drop_na(outcome), 
       aes(y = âge, # variable numérique
           x = résultat)) + # variable de groupe
...
ggplot(
  ...
  scale_fill_manual( # Définir le remplissage pour le fond du violon par mort/récupération
    values = c( "Death" = "#bf5300", 
              "Récupération" = "#11118c")) + 
  scale_color_manual( # Définir les couleurs pour les points par mort/récupération
    values = c( "Death" = "#bf5300", 
              "Récupération" = "#11118c")) + 
...
# Boxplot
boxplot(linelist$wt_kg, main = "A) Boxplot de base") 


# Histogramme
hist(linelist$wt_kg, main = "B) Histogramme de base") 

```

#### Sous-groupes {.unnumbered}  

Les sous-groupes peuvent être créés avec la syntaxe des formules. Notez que dans le graphique B ci-dessous, ```résultat``` et ```sexe``` sont écrits comme ```résultat*sexe```, de sorte que les boxplots existent pour les quatre combinaisons de ces deux colonnes. Ils ne sont pas facettés sur différentes lignes et colonnes comme dans **ggplot2**. Nous spécifions `linelist` à `data = ` pour ne pas avoir à utiliser la syntaxe `$` tout le temps.  

```{r, fig.show='hold', out.width=c('50%', '50%')}

# Box plot par sous-groupe
boxplot(âge ~ résultat,
                  données = linelist, 
                  main = "A) Boxplot de base par sous-groupe")

# Box plot par sous-groupes croisés
boxplot(âge ~ résultat*sexe,
                  données = linelist, 
                  main = "B) Boxplot de base par groupes croisés")
...
boxplot(linelist$age ~ linelist$outcome,
                  ...
                  main="A) Largeur proportionnelle des boxplot()")
