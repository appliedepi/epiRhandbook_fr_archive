...
contain_covid19 <- function(barrier_gest, wear_mask, get_vaccine){
  ...
  else("veuillez vous assurer que toutes les réponses sont oui, cette pandémie doit prendre fin !")
}
...
Dans ce cas, la fonction à écrire en dehors de dplyr serait `fonction (x) (x%%2 == 0)` pour s'appliquer au vecteur contenant tous les numéros de ligne.


``{r, eval=F}
linelist %>%   
   tête de tranche(n=20) %>% 
  ...
  with(mes_données,
       rbind( #liez les résultats des deux fonctions suivantes par ligne
        #tabuler la variable d'intérêt : ne donne que des nombres
          table(mes_données[[var_to_tab]], useNA = "no"),
...
Une manière plus simple serait d'utiliser la base R "apply" au lieu d'une "boucle for" comme exprimé ci-dessous :
...
chart_outcome_province <- function(data_used, prov){
  ...
  highcharter::hchart(
    ...
    name = paste0("Répartition du résultat en :", prov)
    )
...
indic_3 <- flu_china %>% 
  ...
  select(province, age_médian_cas) %>% 
  distinct()
...
contain_covid19_missing <- function(barrier_gest, wear_mask, get_vaccine){
  ...
  else("veuillez vous assurer que toutes les réponses sont oui, cette pandémie doit cesser !")
}
...
contain_covid19_stop <- function(barrier_gest, wear_mask, get_vaccine){
  ...
  if(!is.character(barrier_gest)) (stop("arg1 devrait être un caractère, veuillez entrer la valeur avec `yes`, `no` ou `sometimes"))
