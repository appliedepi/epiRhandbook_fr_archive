...
.libPaths() # Vos chemins de bibliothèques, listés dans l'ordre d'installation/de recherche de R. 
                              ...
                              # menu déroulant des bibliothèques d'installation de paquets) 
```
...
myPaths <- .libPaths() # récupérer les chemins d'accès
myPaths <- c(myPaths[2], myPaths[1]) # les commuter
.libPaths(myPaths) # les réaffecter
```

Si vous avez des difficultés à connecter R Markdown à Pandoc, commencez par ce code pour savoir où RStudio pense que votre installation de Pandoc se trouve.  
...
* Vérifiez l'installation de TinyTex, ou installez TinyTex à l'emplacement C :. Voir la page [R basics] pour savoir comment installer TinyTex.  
...
Une erreur du type : ` ! Paquet pdftex.def Erreur : File 'cict_qm2_2020-06-29_files/figure-latex/unnamed-chunk-5-1.png' not found : using draft setting.` ou `Error : LaTeX n'a pas réussi à compiler nom_fichier.tex.`.  
...
* Maintenant, vous avez deux façons d'accéder au fichier que vous ouvrez. L'utilisation du chemin de la lettre du lecteur devrait fonctionner.  


**Erreur dans install.packages()**  

Si vous obtenez une erreur qui inclut la mention d'un répertoire "verrouillé", par exemple : `Error in install.packages : ERROR : failed to lock directory...``
