...
Pour garder que les lignes uniques d'un tableau de données, utilisez `distinct()` de **dplyr** (comme démontré dans la page [Cleaning data and core functions](https://epirhandbook.com/en/cleaning-data-and-core-functions.html#cleaning-data-and-core-functions)). Les lignes qui sont dupliquées sont enlevées de sorte que seule la première ligne est retenue. Par défaut, la première ligne correspond au plus grand `rownumber` (ordre des lignes de haut en bas). Seules les lignes uniques sont retenues.  
...
Ci-dessus, par exemple, nous pouvons voir que seule la ligne d'Amrish du 5 janvier a été retenue, et que seule la ligne de Brian du 7 janvier a été retenue. Voir les [original data](#dedup_data).   
...
Créez une colonne qui contient une métrique pour la complétude des lignes (pas de valeurs manquantes). Cela peut être utile pour décider des lignes à prioriser par rapport aux autres lors de la déduplication/slicing.  
