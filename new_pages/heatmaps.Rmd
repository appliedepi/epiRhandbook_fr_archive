...
#### Création d'un cadre de données d'infecteurs {.unnumbered} (non numéroté)  
...
### Créer un diagramme de chaleur {.unnumbered} (non numéroté)  

Maintenant, nous pouvons enfin créer le graphique de chaleur avec le paquet **ggplot2**, en utilisant la fonction `geom_tile()`. Consultez la page [ggplot tips] pour en savoir plus sur les échelles de couleur et de remplissage, en particulier la fonction `scale_fill_gradient()`.  
...
ggplot(data = long_prop)+ # utilise des données longues, avec des proportions comme Freq
  geom_tile( # visualisation en tuiles
    aes(
      x = cas, # l'axe des x est l'âge du cas
      y = infecteurs, # l'axe des y est l'âge de l'infecteur
...
Maintenant, le jeu de données a `nrow(agg_weeks)` lignes, alors qu'il avait précédemment `nrow(facility_count_data)`.  
...
expanded_weeks <- agg_weeks %>% 
  ...
  tidyr::expand(., week, location_name) %>% # étendre le cadre de données pour inclure toutes les combinaisons possibles établissement-semaine
                                             # Note : "." représente l'ensemble de données à ce moment de la chaîne de production.
...
Avant d'exécuter ce code, `agg_weeks` contient ` nrow(agg_weeks)` lignes.   
...
Après avoir exécuté ce code, `agg_weeks` contient ` nrow(agg_weeks)` lignes.   
...
Nous pouvons améliorer l'apparence de ce graphique en ajoutant des fonctions **ggplot2** supplémentaires, comme indiqué ci-dessous. Voir la page sur les [astuces ggplot] pour plus de détails.  
...
<!-- ======================================================= -->
### Axe des y ordonné {.non numéroté}  
...
<!-- ======================================================= -->
### Afficher les valeurs {.non numérotées}  
